...
class Grammar(object):
    ...
    def __init__(self, file_address):
        ...
        for i in range(len(self.production_rules)):
            for j in range(len(self.production_rules[i])):
                NTs_to_check_recursiveness = re.findall(
                    r"\<([\(\)\w,-.]+)\>", self.production_rules[i][j][0])
                NTs_to_check_recursiveness = [
                    '<' + item_ + '>' for item_ in NTs_to_check_recursiveness]
                unique_NTs = np.unique(
                    NTs_to_check_recursiveness, return_counts=False)
                recursive = False
                for NT_to_check in unique_NTs:
                    ...
                    if NT_to_check in stack:
                        ...
                        recursive = check_recursiveness(
                            self, NT_to_check, stack)
                        if recursive:
                            break
                        stack.pop()
...
def check_recursiveness(self, NT, stack):
    ...
    for j in range(len(self.production_rules[idx_NT])):
        NTs_to_check_recursiveness = re.findall(
            r"\<([\(\)\w,-.]+)\>", self.production_rules[idx_NT][j][0])
        NTs_to_check_recursiveness = [
            '<' + item_ + '>' for item_ in NTs_to_check_recursiveness]
  ...
  #      while unique_NTs.size and not recursive:
        for NT_to_check in unique_NTs:
            if NT_to_check in stack:
                ...
                # Include the current NT to check it recursively
                stack.append(NT_to_check)
...
def selLexicaseFilterCount(individuals, k):
    ...
    inds_fitness_zero = [
        ind for ind in individuals if ind.fitness.values[0] == 0]
    ...
    for i in range(len(unique_error_vectors)):
        cands = [ind for ind in candidates if ind.fitness_each_sample ==
                 unique_error_vectors[i]]
        # list of lists, each one with the inds with the same error vectors
        candidates_prefiltered_set.append(cands)
    ...
    for i in range(k):
        ...
        while len(cases) > 0 and len(pool) > 1:
            ...
            f = max
            best_val_for_case = f(
                map(lambda x: x.fitness_each_sample[cases[0]], pool))
            pool = [
                ind for ind in pool if ind.fitness_each_sample[cases[0]] == best_val_for_case]
            del cases[0]
...
def mapper_lazy(genome, grammar, max_depth):
    ...
    while next_NT and idx_genome < len(genome):
        NT_index = grammar.non_terminals.index(next_NT)
        # there is a single PR for this non-terminal
...
def random_initialisation(ind_class, pop_size, bnf_grammar,
    ...
    for i in range(pop_size):
        genome = []
        init_genome_length = random.randint(
            min_init_genome_length, max_init_genome_length)
    ...
    for i in range(n_sets_grow):
        ...
        for j in range(set_size):
            ...
            ind = ind_class(genome, bnf_grammar,
                            max_init_depth_, codon_consumption)
...
def crossover_onepoint(parent0, parent1, bnf_grammar, max_depth, codon_consumption,
    ...
    while continue_:
        ...
        new_ind0 = reMap(parent0, new_genome0, bnf_grammar,
                         max_depth, codon_consumption)
        new_ind1 = reMap(parent1, new_genome1, bnf_grammar,
                         max_depth, codon_consumption)
